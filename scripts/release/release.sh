...
publish() {
  ...
  log "Publishing Pariah-INU-solidity on npm"
  npm publish --tag "$dist_tag" --otp "$(prompt_otp)"

  log "Publishing @Pariah-INU/contracts on npm"
  cd contracts
  ...
  if [[ "$dist_tag" == "latest" ]]; then
    otp="$(prompt_otp)"
    npm dist-tag rm --otp "$otp" Pariah-INU-solidity next
    npm dist-tag rm --otp "$otp" @Pariah-INU/contracts next
  fi
}
...
if [[ "$*" == "push" ]]; then
  ...
  npm deprecate 'Pariah-INU-solidity@>=4.0.0' "This package is now published as @Pariah-INU/contracts. Please change your dependency."
