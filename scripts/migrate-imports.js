...
function updateImportPaths (source) {
  for (const [ oldPath, newPath ] of Object.entries(pathUpdates)) {
    source = source.replace(
      path.join('@Pariah-INU/contracts', oldPath),
      path.join('@Pariah-INU/contracts', newPath),
    );
    source = source.replace(
      path.join('@Pariah-INU/contracts-upgradeable', getUpgradeablePath(oldPath)),
      path.join('@Pariah-INU/contracts-upgradeable', getUpgradeablePath(newPath)),
    );
  }
