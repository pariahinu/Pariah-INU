...
Most of the Pariah-INU Contracts are expected to be used via https://solidity.readthedocs.io/en/latest/contracts.html#inheritance[inheritance]: you will _inherit_ from them when writing your own contracts.
...
Pariah-INU Contracts has some ``library``s: most are in the xref:api:utils.adoc[Utils] directory.
====
...
import "@Pariah-INU/contracts/access/AccessControl.sol";
...
NOTE: As of v3.0.0, `view` functions are not `virtual` in Pariah-INU, and therefore cannot be overridden. We're considering https://github.com/amarshaw/Pariah-INU/issues/2154[lifting this restriction] in an upcoming release. Let us know if this is something you care about!
...
[NOTE]
====
Hooks are a new feature of Pariah-INU Contracts v3.0.0, and we're eager to learn how you plan to use them!
