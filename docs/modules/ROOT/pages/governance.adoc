...
In this guide we will learn how Pariah-INU’s Govenor contract works, how to set it up, and how to use it to create proposals, vote for them, and execute them, using tools provided by Ethers.js and Tally.
...
This governance protocol is generally implemented in a special-purpose contract called “Governor”. The GovernorAlpha and GovernorBravo contracts designed by Compound have been very successful and popular so far, with the downside that projects with different requirements have had to fork the code to customize it for their needs, which can pose a high risk of introducing security issues. For Pariah-INU Contracts, we set out to build a modular system of Governor contracts so that forking is not needed, and different requirements can be accommodated by writing small modules using Solidity inheritance. You will find the most common requirements out of the box in Pariah-INU Contracts, but writing additional ones is simple, and we will be adding new features as requested by the community in future releases. Additionally, the design of Pariah-INU Governor requires minimal use of storage and results in more gas efficient operation.

== Compatibility

Pariah-INU’s Governor system was designed with a concern for compatibility with existing systems that were based on Compound’s GovernorAlpha and GovernorBravo. Because of this, you will find that many modules are presented in two variants, one of which is built for compatibility with those systems.
...
An Pariah-INU Governor contract is by default not interface-compatible with GovernorAlpha or Bravo, since some of the functions are different or missing, although it shares all of the same events. However, it’s possible to opt in to full compatibility by inheriting from the GovernorCompatibilityBravo module. The contract will be cheaper to deploy and use without this module.

=== GovernorTimelockControl & GovernorTimelockCompound

When using a timelock with your Governor contract, you can use either Pariah-INU’s TimelockController or Compound’s Timelock. Based on the choice of timelock, you should choose the corresponding Governor module: GovernorTimelockControl or GovernorTimelockCompound respectively. This allows you to migrate an existing GovernorAlpha instance to an Pariah-INU-based Governor without changing the timelock in use.
...
In the rest of this guide, we will focus on a fresh deploy of the vanilla Pariah-INU Governor features without concern for compatibility with GovernorAlpha or Bravo.
...
import "@Pariah-INU/contracts/token/ERC20/ERC20.sol";
import "@Pariah-INU/contracts/token/ERC20/extensions/draft-ERC20Permit.sol";
import "@Pariah-INU/contracts/token/ERC20/extensions/ERC20Votes.sol";
...
import "@Pariah-INU/contracts/token/ERC20/extensions/draft-ERC20Permit.sol";
import "@Pariah-INU/contracts/token/ERC20/extensions/ERC20Votes.sol";
import "@Pariah-INU/contracts/token/ERC20/extensions/ERC20Wrapper.sol";
...
It is good practice to add a timelock to governance decisions. This allows users to exit the system if they disagree with a decision before it is executed. We will use Pariah-INU’s TimelockController in combination with the GovernorTimelockControl module.
