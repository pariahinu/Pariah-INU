...
If your contract is going to be deployed with upgradeability, such as using the xref:upgrades-plugins::index.adoc[Pariah-INU Upgrades Plugins], you will need to use the Upgradeable variant of Pariah-INU Contracts.

This variant is available as a separate package called `@Pariah-INU/contracts-upgradeable`, which is hosted in the repository https://github.com/amarshaw/Pariah-INU-upgradeable[Pariah-INU/Pariah-INU-contracts-upgradeable].
...
TIP: Pariah-INU provides a full suite of tools for deploying and securing upgradeable smart contracts. xref:Pariah-INU::upgrades.adoc[Check out the full list of resources].
...
```console
$ npm install @Pariah-INU/contracts-upgradeable
```

=== Usage

The package replicates the structure of the main Pariah-INU Contracts package, but every file and contract has the suffix `Upgradeable`.

```diff
-import "@Pariah-INU/contracts/token/ERC721/ERC721.sol";
+import "@Pariah-INU/contracts-upgradeable/token/ERC721/ERC721Upgradeable.sol";
